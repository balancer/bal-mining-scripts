...
def compute_bal_for_gas(start_block_timestamp, end_block_timestamp, gas_whitelist, plot=True, verbose=True):
    sql = ''
    with open('src/bal4gas_V2.sql','r') as file:
        sql = (file
            .read()
            .format(start_block_timestamp, 
                end_block_timestamp, 
                '\',\''.join(gas_whitelist)))
    if verbose: print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) + ' - Querying Bigquery for eligible V2 swaps and reimbursement values ...')
    ...
    if verbose: print(f'ETH reimbursements for the week (V2): {sum(reimbursements.eth_reimbursement)}')
    ...
    if verbose: print(f'BAL reimbursements for the week (V2): {sum(merge.bal_reimbursement)}')

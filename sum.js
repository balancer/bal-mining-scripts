    ...
    try {
        ...
        for (i = END_BLOCK; i > START_BLOCK; i -= BLOCKS_PER_SNAPSHOT) {
            ...
            Object.keys(report).forEach(user => {
                ...
                if (userTotals[user]) {
                    userTotals[user] = bnum(userTotals[user]).plus(bnum(report[user])).toString()
                } else {
        ...
        console.log(balTotal.toString())
        fs.writeFileSync(`./_totals.json`, JSON.stringify(userTotals, null, 4));
